...
const resolverInscripciones = {
  ...
  //       _id: parent.estudiante.toString(),
  //     });
  //     return usr;
  //   },
  // },
    ...
    InscripcionesAvance: async (parent, args, context) => {
      ...
      if (context.userData) {
        if (context.userData.rol === "ESTUDIANTE") {
          ...
          const proyectos = await ProjectModel.find({ ...filtro1 })
            .populate("lider")
            .populate("avances");
          return proyectos;
...
=======
      if (context.userData.rol == "ESTUDIANTE") {
        const proyectoInscribir = await ProjectModel.find({
          _id: args.proyecto,
        });
...
>>>>>>> 9b1d28894f1a0fb70f94bdc4ceb5197b32789c02
    },
    ...
    editarInscripcion: async (parent, args) => {
      const inscripcionEditado = await InscriptionModel.findByIdAndUpdate(
        args._id,
        {
          proyecto: args.proyecto,
        }
    ...
    eliminarInscripcion: async (parent, args) => {
      const inscripcionEliminada = await InscriptionModel.findOneAndDelete({
        _id: args._id,
      });
    ...
    egresarInscripcionesFaseProyecto: async (parent, args) => {
      ...
      console.log(
        "esto regra egresar inscripciones",
        inscripcionesEgresadasFase
      );
